...
import {alias} from '../../utils/cacx.js'
...
EveService.getPrice = async function (isEur, name) {
    ...
    let aliasJson = await alias(name)
    name = aliasJson['data']
    let nameJson = await post(url + 'searchname', {'name': name})
    if ((!nameJson || nameJson.length === 0) && name.endsWith('级')) {
    }


    for (const item of nameJson) {
        if (item['typename'].indexOf('涂装') !== -1) {
            name = name.endsWith('级') ? name : name + '级'
            let typeId = await get(BotConfig.app.corp.path + 'api/getTypeId?typeName=' + encodeURIComponent(name))
            nameJson.push({
                'typeid':typeId ,
                'typename': name
            })
    ...
    if (result.length === 1) {
        ...
        if(typeof historyJson === 'object'){
            console.log(historyJson)
            historyJson = historyJson['result']['openHistory']
            console.log(historyJson)
            for (let item of historyJson) {
                ...
                time.push(item['date'])
                sell.push(item['sellPriceMax'])
                buy.push(item['buyPriceOpen'])
            }
        }
