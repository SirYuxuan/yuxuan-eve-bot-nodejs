...
export async function price(e, {Models}) {
    ...
    let priceJson = await get('http://api.hd-eve.com/bot/getPrice/'+isEur+'/'+(checkRet[1].indexOf('col') !== -1)+"/" + checkRet[2].trim())
    if(priceJson['code'] !== 200){
        e.reply(priceJson['msg'])
        return false
    }
    let dataJson = priceJson['data']
    if(!dataJson['history']){
